...
export class MediasoupProducerWebRTCTransport extends MediasoupWebRTCTransportManager {
  ...
  async webRTCTransportRestartIce({ transportId }: { transportId: string }) {
    console.log("%c mediasoup.producer.webrtc.transport.service webRTCTransportRestartIce");

    // 从缓存中取出 transport
    const webRTCTransport = MediasoupProducerWebRTCTransport.transports.get(transportId);
  ...
  async getWebRTCTransport(data: { roomId: string }) {
    const mediasoupProducerWebRTCTransports = MediasoupProducerWebRTCTransport.transports.keys()
