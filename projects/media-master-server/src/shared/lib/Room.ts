...
class Room extends EventEmitter
{
	...
	private _broadcasters: Map<string, any>; // 或者具体的类型替代any
	...
	static async create({ mediasoupWorker, roomId })
	{
		...
		const protooRoom = new protoo.Room();//创建protoo的Room实例

		// Router media codecs.
		const { mediaCodecs } = config.mediasoup.routerOptions;//从配置中获取支持的媒体编码

		// Create a mediasoup Router.
		const mediasoupRouter = await mediasoupWorker.createRouter({ mediaCodecs });//创建mediasoupRouter
	...
	async createBroadcaster({ id, displayName, device = {}, rtpCapabilities })
	{
		...
		else if ((device as any).name!=='string' || !(device as any).name)
		...
		const broadcaster =
		{
			...
			data :
			{
				...
				device :
				{
					...
					name    : (device as any).name || 'Unknown device',
		...
		switch (request.method)
		{
			...
			case 'join':
			{
				...
				for (const joinedPeer of joinedPeers)
				{
					...
					for (const producer of joinedPeer.data.producers.values())
					{
						this._createConsumer(//所有现存的生产者，都创建对此新连接进来的人的消费渠道，
						//效果：其他人可以看到新进来的人的画面
							{
								consumerPeer : peer,
					...
					for (const dataProducer of joinedPeer.data.dataProducers.values())
					{
						...
						this._createDataConsumer(//所有的现存的数据生产者，都创建对此新连接进来的人的数据消费渠道
						//效果：其他人可以收到新进来的人的消息
							{
								dataConsumerPeer : peer,
				...
				this._createDataConsumer(//为机器人数据生产者创建对此新连接进来的人的数据消费渠道
				//效果：机器人可以收到新进来的人的消息
					{
						dataConsumerPeer : peer,
				...
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer }))
				{
					otherPeer.notify(//将有新的人进入房间的事情通知房间内的其他人
						'newPeer',
						{
							id          : peer.id,
			...
			case 'createWebRtcTransport':
			{
				...
				if (forceTcp)
				{
					...
					(webRtcTransportOptions as any).enableUdp = false;
					(webRtcTransportOptions as any).enableTcp = true;
				}

				const transport = await this._mediasoupRouter.createWebRtcTransport(//调用mediasoup创建一个webrtc传输
					webRtcTransportOptions);

				transport.on('sctpstatechange', (sctpState) =>//注册sctp状态改变事件
				{
					logger.debug('WebRtcTransport "sctpstatechange" event [sctpState:%s]', sctpState);
				});

				transport.on('dtlsstatechange', (dtlsState) =>//注册dtls状态改变事件
				{
			...
			case 'produce':
			{
				...
				const producer = await transport.produce(//传输生产数据（音视频数据）
					{
						kind,
						rtpParameters,
						appData
				...
				// Set Producer events.
				producer.on('score', (score) =>//收到传输质量分数，说明传输进行中
				{
					// logger.debug(
					// 	'producer "score" event [producerId:%s, score:%o]',
					// 	producer.id, score);

					peer.notify('producerScore', { producerId: producer.id, score })//通知生产者，传输质量分数
						.catch(() => {});
				});

				producer.on('videoorientationchange', (videoOrientation) =>//收到视频方向改变事件
				{
					logger.debug(
				...
				producer.on('trace', (trace) =>//收到跟踪事件
				{
					logger.debug(
				...
				// Add into the audioLevelObserver.
				if (producer.kind === 'audio')//如果是音频类型，在音频级别观察者中添加生产者，效果：当生产者发送音频时，其他Peer将收到音频级别事件？
				{
					this._audioLevelObserver.addProducer({ producerId: producer.id })
						.catch(() => {});
				}
	...
	async _createConsumer({ consumerPeer, producerPeer, producer })
	{
		...
		const transport = Array.from(consumerPeer.data.transports.values()).find((t) => (t as any).appData.consuming);
		...
		})
	{
		...
		const transport = Array.from(dataConsumerPeer.data.transports.values())
			.find((t) => (t as any).appData.consuming);
