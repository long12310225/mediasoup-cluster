...
export default class Room extends EventEmitter {
  ...
  constructor({ roomId, mediasoupRouter }) {
    ...
    this._protooRoom = new protoo.Room(roomId);
  ...
  async _init() {
    ...
    const audioLevelObserver = await this._mediasoupRouter.createAudioLevelObserver({
      maxEntries: 1,
      threshold: -80,
      interval: 800,
    });
    ...
    const bot = await Bot.create({
      mediasoupRouter: this._mediasoupRouter
    });
    this._bot = bot

    // For debugging.
    global.audioLevelObserver = audioLevelObserver
    global.bot = bot
  ...
  /**
   * 与浏览器连接
   * Called from server.js upon a protoo WebSocket connection request from a browser.
   *
   ...
   * @param {protoo.WebSocketTransport} protooWebSocketTransport - The associated protoo WebSocket transport.
   */
  handleProtooConnection({ peerId, consume, protooWebSocketTransport }) {
    ...
    peer.on('request', (request, accept, reject) => {
      // TODO
      console.debug('protoo Peer "request" event [method:%s, peerId:%s]', request.method, peer.id)
   ...
   */
  async createBroadcaster({ id, displayName, device = {}, rtpCapabilities }: any) {
  ...
  /**
   * 创建 mediasoup Transport Broadcaster
  ...
  async createBroadcasterTransport({ broadcasterId, type, rtcpMux = false, comedia = true, sctpCapabilities }) {
    ...
    switch (type) {
      case 'webrtc': {
        ...
        {
          ...(JSON.parse(env.getEnv('WEB_RTC_TRANSPORT_OPTIONS'))),
          listenIps: JSON.parse(env.getEnv('MEDIASOUP_WEBRTC_TRANSPORT_LISTEN_IPS')),
      ...
      case 'plain': {
        const plainTransportOptions = {
          ...(JSON.parse(env.getEnv('PLAIN_TRANSPORT_OPTIONS'))),
          rtcpMux: rtcpMux,
          comedia: comedia,
        }
  ...
  /**
   * 连接一个 Broadcaster mediasoup WebRtcTransport
  ...
  /**
   * 连接一个 Broadcaster mediasoup PlanTransport
   ...
   * @type .
   */
  async connectBroadcasterPlainTransport({ broadcasterId, transportId, ip, port, rtcpport }) {
    const broadcaster = this._broadcasters.get(broadcasterId)
    ...
    //	}
    console.info('do connect ip %s rtpport %d,rtcpport %d', String(ip), port, rtcpport)
  ...
  /**
   * 创建一个 mediasoup Producer Broadcaster
  ...
  /**
   * 创建一个 mediasoup Consumer Broadcaster
   ...
   * @type {String} producerId
   */
  async consumerRsume({ broadcasterId, consumeId }) {
    const broadcaster = this._broadcasters.get(broadcasterId)
    ...
    console.info('get consumer success consumerid %s', String(consumeId))
    setTimeout(async () => {
  ...
  /**
   * 创建一个 mediasoup Broadcaster DataConsumer
  ...
  /**
   * 创建一个 mediasoup Broadcaster DataProducer
  ...
  async _handleProtooRequest(peer, request, accept, reject) {
    switch (request.method) {
      // 001
      case 'getRouterRtpCapabilities': { //获取路由的rtp能力
        accept(this._mediasoupRouter.rtpCapabilities) //这个是create的时候创建的mediasoupRouter，然后从构造函数传入成员变量_mediasoupRouter
        break
      }
      // 002、003
      case 'createWebRtcTransport': {
        ...
        const { maxIncomingBitrate } = JSON.parse(env.getEnv('WEB_RTC_TRANSPORT_OPTIONS'))
      ...
      case 'join': {
        ...
        accept({ peers: peerInfos }) //这里得到的是除了请求者之外的其他人
      ...
      case 'connectWebRtcTransport': {
        ...
        const transport = peer.data.transports.get(transportId) //通过id取得transport

        if (!transport) throw new Error(`transport with id "${transportId}" not found`)

        await transport.connect({ dtlsParameters }) //连接传输
      ...
      case 'produce': {
        ...
        // Optimization: Create a server-side Consumer for each Peer.
        //优化:为每个Peer创建一个服务器端消费者。效果：在服务端创建了消费者，这样可以转发音视频数据给客户端
        for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
          this._createConsumer({
      ...
      case 'restartIce': {
        ...
        const iceParameters = await transport.restartIce() //重启ICE协商
      ...
      case 'setConsumerPreferredLayers': {
        ...
        await consumer.setPreferredLayers({ spatialLayer, temporalLayer }) //设置消费首选图层
      ...
      case 'setConsumerPriority': {
        ...
        await consumer.setPriority(priority) //设置消费优先级
      ...
      case 'requestConsumerKeyFrame': {
        ...
        await consumer.requestKeyFrame() //请求消费关键帧
      ...
      case 'applyNetworkThrottle': { //应用网络节流
      ...
      case 'resetNetworkThrottle': { //重置网络节流
  ...
  async _createConsumer({ consumerPeer, producerPeer, producer }) {
    ...
    const transport: any = Array.from(consumerPeer.data.transports.values()).find((t: any) => t.appData.consuming)
  ...
  async _createDataConsumer({
    ...
    // Must take the Transport the remote Peer is using for consuming.
    const transport: any = Array.from(dataConsumerPeer.data.transports.values()).find((t: any) => t.appData.consuming)
