...
const workers = new Map() //工作函数
const routers = new Map() //路由
const transports = new Map() //传输通道
const producers = new Map() //生产者
const consumers = new Map() //消费者
const dataProducers = new Map() //数据生产者
const dataConsumers = new Map() //数据消费者
...
class Interactive {
  ...
  openCommandConsole() {
    ...
    /*
			创建一个命令行接口，允许通过input和output属性指定的_socket进行交互
		*/
    const cmd = readline.createInterface({
      input: this._socket,
      output: this._socket,
      terminal: true, //表示这个接口应该模拟一个终端
    })

    /*
			监听close事件，当用户操作结束，例如Ctl+C之类的，会触发
		*/
    cmd.on('close', () => {
